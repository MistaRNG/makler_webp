...
#define BUTTON1 _BV(PD5) // Taster 1 auf PD5
#define BUTTON2 _BV(PD6) // Taster 2 auf PD6
#define BUTTON3 _BV(PD7) // Taster 3 auf PD7
...
void init_clock(void) {
    HOUR_LEDS_DDR |= 0x1F;                      // Stunden-LEDs als Ausgang - 00011111
    MINUTE_LEDS_DDR |= 0x3F;                    // Minuten-LEDs als Ausgang - 00111111

    BUTTON_DDR_D &= ~((1 << PD5) | (1 << PD6) | (1 << PD7)); // Taster an PD5, PD6 und PD7 als Eingang
    BUTTON_PORT_D |= (BUTTON1 | BUTTON2 | BUTTON3);          // Pull-up Widerstände der Taster aktivieren

    PCICR |= (1 << PCIE0);                      // Pin-Change-Interrupts aktivieren (Notwendig für das Aufwachen)
    PCMSK0 |= (1 << PCINT0);                    // Pin-Change-Interrupts für PB0

    DDRB |= (1<<0);                             // Setze PD0 als Ausgang fuer die Zeitmessung
}
...
void display_time() {
    ...
    for (uint8_t i = 0; i < 5; i++) {           // Durch die Fuenf Stunden-LEDs iterieren
        if (currentTime.hours & (1 << i)) {     // Die Bits der aktuellen Zeit werden der Reihe nach ausgelesen
            HOUR_LEDS_PORT |= (1 << (PD4 - i)); // ... und auf die Stunden LEDs geschrieben
        }
    }
...
void all_leds_on() {
    PORTC |= 0x1F; //00111111 - Minuten-LEDs
